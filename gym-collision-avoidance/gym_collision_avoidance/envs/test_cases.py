...
def get_testcase_two_agents(start=[[0,0],[5,0]],goal=[[5,3],[0,3]],policies=['learning', 'GA3C_CADRL']):
    goal_x = 3
    goal_y = 3
    agents = [
        Agent(start[0][0], start[0][1], goal[0][0], goal[0][1], 0.5, 1.0, 0.0, policy_dict[policies[0]], UnicycleDynamics, [OtherAgentsStatesSensor], 0),
        Agent(start[1][0], start[1][1], goal[1][0], goal[1][1], 0.5, 1.0, np.pi, policy_dict[policies[1]], UnicycleDynamics, [OtherAgentsStatesSensor], 1)
        ]
    return agents
